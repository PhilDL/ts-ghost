...
_Documentation here may be outdated, this package being internal, it is changing a lot through versions to versions... still under construction 🏗️_
...
- last argument is the `HTTPClient` instance that will be used to do the actual `fetch` and handle the JWT if using the admin endpoint.
